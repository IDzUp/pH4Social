<?php

return array(
    'Hello' => 'Γεια σας',
    'Already a member?' => 'Είστε ήδη μέλος;',
    'Sign In' => 'Είσοδος',
    'Profile' => 'Προφίλ',
    'Logout' => 'Αποσύνδεση',
    'Lorem ipsum dolor' => 'Lorem ipsum dolor',
    'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Mauris at nibh porttitor,
          iaculis lacus vitae, varius enim. Praesent fermentum egestas volutpat. Cras tempor finibus magna, non volutpat odio feugiat quis. Nam bibendum mi tortor, in auctor felis laoreet at. Pellentesque
          aliquet orci ac mollis congue.' => 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Mauris at nibh porttitor,
          iaculis lacus vitae, varius enim. Praesent fermentum egestas volutpat. Cras tempor finibus magna, non volutpat odio feugiat quis. Nam bibendum mi tortor, in auctor felis laoreet at. Pellentesque
          aliquet orci ac mollis congue.',
    '© besocial.com 2014. All Rights Reserved.' => '© besocial.com 2014. All Rights Reserved.',
    'Forget Password' => 'Ξεχάστε τον κωδικό σας',
    'Login' => 'Είσοδος',
    'Meet new people' => 'Γνώρισε νέα άτομα',
    'Friend Request' => 'Αίτημα φίλο',
    'Select Your Language' => 'Επιλέξτε τη γλώσσα σας',
    'Reset Your Password' => 'Επαναφορά του κωδικού πρόσβασης',
    'Send' => 'Αποστολή',
    'Sign up' => 'Εγγραφείτε',
    'Gender' => 'γένος',
    'Female' => 'θηλυκός',
    'Male' => 'αρσενικός',
    'Notifications' => 'Ειδοποιήσεις',
    'Inbox' => 'Εισερχόμενα',
    'Settings' => 'Ρυθμίσεις',
    'Messages' => 'Μηνύματα',
    'Logout' =>  'Αποσύνδεση',
    'Friends List' => 'Λίστα Φίλων',
    'Gallery' => 'Gallery',
    'Upload a Song' => 'Ανεβάστε ένα τραγούδι',
    'Create a Listing' => 'Δημιουργήστε μία Καταχώρηση',
    'Create a Page' => 'Δημιουργήστε μια Σελίδα',
    'Share' => 'Share',
    'Submit' => 'Υποβολή',
    'edit' => 'edit',
    'Create Event' => 'Δημιουργία συμβάντος',
    'Upcoming Events' => 'Προσεχείς Εκδηλώσεις',
    'Chat Messenger' => 'Chat Messenger',
    'Reply' => 'απάντηση'





);