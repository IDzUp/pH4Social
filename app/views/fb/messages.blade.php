...
else
{
 ...
 $.ajax({
    ...
    type: "GET",
    url: "/messagestore/"+id,
    data: { id: id, user: user, comment: comment, otheruser: otheruser, name: name, othername: othername},
    dataType: 'json',
    cache: false,
    success: function(data)
    {
    ...
    {?>
       <div class="media border-bottom margin-none bg-gray pull-right" id="messagebox">
              <a class="pull-right innerAll" href=""><img width="50" class="media-object"
                src="{{ asset('/') }}/{{$mainuserprofile}}" style="width: 50px; height: 50px; display: block; margin-left: auto; margin-right: auto;">
              </a>
              <div class="media-body innerTB pull-right">
                  <a class="strong text-inverse" href="{{ asset('index.php') }}/profile">{{$hello->name}}</a>  <small class="text-muted ">wrote on {{$hello->timess}}</small> <a class="text-small" href=""></a>
                  <div>{{$hello->chat}}</div>
              </div>
          </div>
...
{?>
  <div class="media border-bottom margin-none bg-gray pull-left">
              <a class="pull-left innerAll" href=""><img width="50" class="media-object"
                src="{{ asset('/') }}/{{$otheruserprofile}}" style="width: 50px; height: 50px; display: block; margin-left: auto; margin-right: auto;"></a>
              <div class="media-body innerTB">
                  <a class="strong text-inverse" href="{{ asset('index.php') }}/profileopen/<?php foreach($alluser as $all){ if($all->rand==$hello->user) { echo $all->uname; } } ?>">{{$hello->name}}</a>  <small class="text-muted ">wrote on {{$hello->timess}}</small> <a class="text-small" href=""></a>
 ...
 -->
      ...
      <input type="hidden" id="othername" value="{{$proinfo->name;}}" name="othername">
       <input type="hidden" id="name" value="{{Auth::user()->name; }}" name="name">
        ...
        <input type="hidden" id="otheruser" value="<?php echo $proinfo['rand']; ?>" name="otheruser">

      <textarea rows="3" onkeydown="if (event.keyCode == 13) document.getElementById('submitbtn').click()" required="" class="form-control margin-top" name="chat" id="chat" placeholder="Write a reply"></textarea>
    <div class="highlight">
    <button id="submitbtn" class="like_btn mar_top" type="button"><span class="glyphicon glyphicon-share"></span> Reply</button>
     </div>
       </form>
