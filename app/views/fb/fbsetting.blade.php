      ...
      <div class="col-sm-4 col-md-4">
         <div class="thumbnail center_box">
          <div class="friend_list_img">
           <img class="img-circle" alt="" src="{{ asset('/') }}public/{{ $hello->profile }}">
            </div>
            <div class="caption">
             <h3>{{ $hello->name }}</h3>
            <a href="{{ asset('/index.php/') }}/unblock/<?php echo $hello->uname; ?>" class="like_btn btn-lg mar_top" type="button">UnBlock</a>
          </div>
        </div>
     </div>
@endforeach
...
@endif
    {{ Form::token() }}
       ...
       <input type="hidden" name="id" value="<?php echo Auth::user()->id;?>"/>
        <input type="radio" <?php if($online->chat==0){ ?>checked="" <?php } ?> name="chat" value="0"/>  Offline *
      </label>
      <label class="checkbox-inline">
        <input type="radio" <?php if($online->chat==1 || $online->chat==null){ ?>checked="" <?php } ?>name="chat" value="1"/> Online *
       </label>
       <label class="checkbox-inline">
...
@endif
    {{ Form::token() }}
       <label class="checkbox-inline">
       <input type="hidden" name="id" value="<?php echo Auth::user()->id;?>"/>
    <input type="radio" <?php if($account->account==1 || $account->account==null){ ?>checked="" <?php } ?>name="account" value="1"/> Activate
      </label>
      <label class="checkbox-inline">
       <input type="radio" <?php if($account->account==0){ ?>checked="" <?php } ?> name="account" value="0"/> Deactivate
       </label>
       <label class="checkbox-inline">
...
@endif
    {{ Form::token() }}
       <label class="checkbox-inline">
       <input type="hidden" name="id" value="<?php echo Auth::user()->id;?>"/>
    <input type="radio" <?php if($account->visitors==1 || $account->visitors==null){ ?>checked="" <?php } ?>name="account" value="1"/> Public
      </label>
      <label class="checkbox-inline">
       <input type="radio" <?php if($account->visitors==0){ ?>checked="" <?php } ?> name="account" value="0"/> Only Friends
       </label>
     <label class="checkbox-inline">
        <button class="like_btn" type="submit">Submit</button>
        </label>
