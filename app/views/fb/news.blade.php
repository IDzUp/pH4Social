...
{{ HTML::script('public/jsfb/jquery.form.js') }}
...
$('#percentage').text('');
  ...
  //$('label#coverimage').attr('style','background:none repeat scroll 0 0 / 100% 100% #333;');

//$('nav ul li').attr('style','background:none repeat scroll 0 0 / 100% 100% #333;');
...
$('label#imagessvideo').css('background-image','url(../public/imagesfb/cameras.png)');
...
var datas='<div class="message_box" id="post'+insert_id+'"><div id="fleft'+insert_id+'"><div class="u-nformation"><img  alt="iamge" class="img-circle img_wid myimgs" src="{{ asset('') }}/'+profile+'"><a href="{{ asset("index.php") }}/profileopen/<?php  echo Auth::user()->id;?>"><?php  echo Auth::user()->name;?></a><button id="'+insert_id+'" class="postdelete pull-right"><span class="glyphicon glyphicon-remove"></span></button></div><object width="500" height="281"><param name="allowfullscreen" value="true" /><param name="allowscriptaccess" value="always" /><param name="movie" value="'+vimeo+'" /><embed src="'+vimeo+'" type="application/x-shockwave-flash" allowfullscreen="true" allowscriptaccess="always" width="500" height="281"></object><div class="message_text1"><p id="edit'+insert_id+'">'+post+'</p><button id="'+insert_id+'" class="postedit ed'+insert_id+'"><span class="">edit</span></button></div><div class="comment_box"> <span class="date_text">'+curdate+'</span><div class="like"><ul><li><span class="glyphicon glyphicon-comment"></span></li><li><span id="countlike'+insert_id+'"></span></li><li><span><a style="cursor:pointer;" class="likes glyphicon glyphicon-thumbs-down" id="a'+insert_id+'" name="'+insert_id+'"></a></span></li></ul></div></div></div><div id="rleft'+insert_id+'"><div class="cl'+insert_id+'" style="display:none; float:right;"><a style="cursor:pointer;" name="'+insert_id+'" class="clx"><span class="glyphicon glyphicon-remove"></span></a></div><div class="message_text" id="ppcomment'+insert_id+'"></div><div class="comment_box comment_boxs"><form onsubmit="return comm('+insert_id+',this)" name="comments" id="uni'+insert_id+'" method="post" enctype="multipart/form-data" >{{ Form::token() }}<textarea style="width:98%; max-width:98%;" name="comment"  onkeydown="if (event.keyCode == 13) document.getElementById("submitbtn'+insert_id+'").click()" required="" id="comment'+insert_id+'" value="" placeholder="comment"></textarea><input type="hidden" name="user" id="user'+insert_id+'" value="<?php  echo Auth::user()->rand;?>"/><input type="hidden" id="post_id'+insert_id+'" name="post_id" value="'+insert_id+'"/><input type="hidden" name="name" id="name'+insert_id+'" value="<?php  echo Auth::user()->name;?>"/><label id="imagess" style="cursor:pointer;" class="imgg'+insert_id+'"><input class="imggs'+insert_id+'" id="imagesss" name="imagesss"  type="file"></label><label id="imagessvideo" style="cursor:pointer;" class="vdo'+insert_id+'"><input class="vdos'+insert_id+'" id="imagessvideo" name="imagessvideo"  type="file"></label><button  id="submitbtn'+insert_id+'" type="submit" class="buttonss'+insert_id+' btn send_btn pull-right">send</button></form></div></div></div>';
$("#postapend").prepend(datas);
...
//$(document).ready(function (e) {
 ...
 //$( document ).on( "click",".coms", function(e) {
...
#percentage {
  ...
  <div class="container">
    <div class="row">
      <div class="cover" style="width:1170px; height:334px;">


@foreach( $profile as $hello)

<img src="{{ asset('/') }}{{$hello->cover}}" width="1170" height="334" alt=""  class="img-rounded"/>
...
<label id="coversimage">
        Add a Cover Photo
                <input id="coversimages" name="image"  required="" tabindex="1" type="file" onchange="displayPreview();">
      ...
      {{ Form:: close() }}
  <a id="remo" class="<?php echo Auth::user()->uname; ?>" style="color:#333; <?php if($photos==0){ echo 'pointer-events:none';} ?> @foreach( $profile as $hello) <?php if($hello->cover=='imagesfb/male_cover.jpg' || $hello->cover=='imagesfb/female_cover.jpg'){ echo 'pointer-events:none'; } ?> @endforeach">
...
<section class="user_sec">
  <div class="container">
   @if (Session::has('large'))
   <div class="alert alert-danger" role="alert">{{ Session::get('large') }}</div>
@endif
    <div class="row">
      <div class="col-lg-2 col-md-3 col-sm-3">
        <div class="row">
          ...
          <div class="nav">
            <div class="userphoto">
         <a id="remos" style="padding:0px; border-bottom:none; <?php if($photos==0){ echo 'pointer-events:none';} ?> @foreach( $profile as $hello) <?php if($hello->path=='imagesfb/male.jpg' || $hello->path=='imagesfb/female.jpg'){ echo 'pointer-events:none'; } ?> @endforeach"  class="<?php echo Auth::user()->uname;?>">
    <span title="remove profile photo" style="cursor:pointer; color:red; float:right;" class="glyphicon glyphicon-remove"></span>
</a>
@foreach( $profile as $hello)
<img src="{{ asset('/') }}{{$hello->path}}" alt="" class="img-circle" width="170" height="170"/>
...
<img src="{{ asset('/') }}imagesfb/{{$male}}" width="170" height="170" alt=""  class="img-circle"/>
<?php
}
...
@endif
    {{ Form::token() }}
          ...
          <input id="coverimagek" name="image" required="" tabindex="1" type="file" onchange="document.getElementById('submit').click();">
                </label>
      ...
      {{ Form:: close() }}
              ...
              <li><a href="{{ asset('/index.php/') }}/friendlist"><span class="glyphicon glyphicon-tasks"></span> {{trans ('greeting.Friends List')}}</a> </li>
              <li><a href="{{ asset('/index.php/') }}/gallery"><span class="glyphicon glyphicon-calendar"></span> {{trans ('greeting.Gallery')}}</a></li>
               <li class="{{ Request::is( 'news') ? 'activess' : '' }}" ><a href="{{ asset('/index.php/') }}/news"><span class="glyphicon glyphicon-calendar"></span> News Feed</a></li>
...
@endif
    {{ Form::token() }}
          ...
          <!--   <li> <a href="#profile" role="tab" data-toggle="tab"> <span class="glyphicon glyphicon-map-marker"></span> </a> </li> -->
             ...
             <label id="imagess" style="cursor:pointer;" class="mainimg" ><input class="mainimgs" id="imagesss" name="image"  type="file"></label>
             </span>
             ...
             <label id="imagessvideok" style="cursor:pointer;" class="mainvdo"><input class="mainvdos" id="imagessvideo" name="imagessvideo"  type="file"></label>
             </span>
          ...
          <div class="tab-content">
            ...
            <div class="tab-pane active" id="home">
              <textarea style="max-width:616px;" id="textpost" required="true" name="post" class="form-control" rows="3"></textarea>
            </div>
...
var intervalss = 20 * 60 * 8; // where X is your every X minutes
...
{
var datas='<div class="media border-bottom margin-none bg-gray pull-right" id="messagebox"><a class="pull-right innerAll" href=""><img width="50" class="media-object" src="{{ asset('') }}/'+data[i].mainuserprofile+'" style="width: 30px; height: 30px; display: block; margin-left: auto; margin-right: auto;"></a><div class="media-body innerTB pull-right"><a class="text-small" href=""></a><div>'+data[i].chat+'</div><small class="text-muted ">'+data[i].timess+'</small> </div></div><div class="clearfix"></div>';
...
var datas='<div class="media border-bottom margin-none bg-gray pull-left"><a class="pull-left innerAll" href=""><img width="50" class="media-object" src="{{ asset('') }}/'+data[i].otheruserprofile+'" style="width: 30px; height: 30px; display: block; margin-left: auto; margin-right: auto;"></a><div class="media-body innerTB"><a class="text-small" href=""></a><div>'+data[i].chat+'</div><small class="text-muted ">'+data[i].timess+'</small> </div></div><div class="clearfix"></div> ';
var blankid='#uptextid'+data[i].idss;
$(''+blankid+'').append(datas);
...
<div id="fleft{{$hello['id']}}">
          <div class="u-nformation"><img  alt="image" class="img-circle img_wid  <?php if($hello['user_rand']==Auth::user()->rand) { echo 'myimgs'; } ?>" src="{{ asset('/') }}<?php if($allimg) { foreach($allimg as $img){ if($img->user_rand== $hello['user_rand']) {  $mm=$img->thumb; echo $mm; break; } else { $mm=''; } } } if($mm==''){ foreach($alluser as $all) { if($all->rand==$hello['user_rand']) { if($all->sex=='male') { echo 'imagesfb/male.jpg'; } else { echo 'imagesfb/female.jpg'; } } }  } ?>">

           <a href="{{ asset('index.php') }}/profileopen/<?php foreach($alluser as $img){ if($img->rand== $hello['user_rand']) { echo $img->uname; }  } ?>"><?php foreach($alluser as $img){ if($img->rand== $hello['user_rand']) { echo $img->name; }  } ?></a>
    ...
    {?>
<button id="{{$hello['id']}}" class="postdelete pull-right"><span class="glyphicon glyphicon-remove"></span></button>
...
<embed src="{{$hello['imagessvideo']}}" type="application/x-shockwave-flash" allowfullscreen="true" allowscriptaccess="always" width="500" height="281">
...
else
{
  ...
  <source src="<?php if($test==0) { echo asset('/'); } ?>{{$hello['imagessvideo']}}" type="video/mp4">
  <source src="<?php if($test==0) { echo asset('/'); } ?>{{$hello['imagessvideo']}}" type="video/ogg">
...
<?php
    ...
    if(Auth::user()->username=='admin' || $hello['user_rand']==Auth::user()->rand)
    {?>
            <button id="{{$hello['id']}}" class="postedit ed{{$hello['id']}}"><span class="">{{trans ('greeting.edit')}}</span></button>
          <?php
        }
        ?>
...
<div class="cl{{$hello['id']}}" style="display:none; float:right;"><a style="cursor:pointer;" name="{{$hello['id']}}" class="clx"><span class="glyphicon glyphicon-remove"></span></a></div>
...
<div class="clfcc{{$hellos->id}}" style="display:none; float:right;"><a style="cursor:pointer;" name="{{$hellos->id}}" class="clxs"><span class="glyphicon glyphicon-remove"></span></a></div>
...
<button id="{{$hellos->id}}" class="comedit eds{{$hellos->id}}"><span class="">{{trans ('greeting.edit')}}</span></button>
...
<form  onsubmit="return comm({{$hello['id']}},this)" name="comments{{$hello['id']}}" data-value="{{$hello['id']}}" class="com" id="uni{{$hello['id']}}" method="post" enctype="multipart/form-data" >
  {{ Form::token() }}

<textarea style="width:98%; max-width:98%;" name="comment"  onkeydown="if (event.keyCode == 13) document.getElementById('submitbtn{{$hello['id']}}').click()" required="" id="comment{{$hello['id']}}" value="" placeholder="comment"></textarea>
...
<label id="imagess{{$hello['id']}}" style="cursor:pointer;" class="imgg"><input onchange="image({{$hello['id']}});" class="imggs" id="imagesss{{$hello['id']}}" name="imagesss"  type="file"></label>


<label id="imagessvideos{{$hello['id']}}" style="cursor:pointer;" class="vdo"><input onchange="video({{$hello['id']}});" class="vdos" id="imagessvideo{{$hello['id']}}" name="imagessvideo"  type="file"></label>


<button  value="{{$hello['id']}}" name="{{$hello['id']}}" id="submitbtn{{$hello['id']}}" type="submit" class="buttonss{{$hello['id']}} btn send_btn pull-right coms">{{trans ('greeting.Send')}}</button>
</form>
</div>
...
<button value="{{$hello['id']}}" id="submitbtn{{$hello['id']}}" type="submit" class="buttonss{{$hello['id']}} btn send_btn pull-right coms">{{trans ('greeting.Send')}}</button>
</form>
</div>
...
<!-- <a style="cursor:pointer;" class="nextload">Next load</a> -->
      </div>
    ...
    {{ Form::token() }}
            ...
            <div class="form-group padding">
              <div class='input-group date' id='datetimepicker1'>
                <input type='text' required="" placeholder="Date and Time" readonly="readonly" name="dates" class="form-control" />
                <span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span> </span> </div>
              <div class="input-group padding">
                <input type="text" required pattern=".*\S+.*" placeholder="Event Name" name="event" class="form-control">
                <span class="input-group-addon glyphicon glyphicon-pencil"></span> </div>
 <div class="input-group padding">
                <input type="text" required pattern=".*\S+.*" placeholder="Location" name="address" class="form-control">
...
?>
          ...
          <div class="col-lg-12 col-md-12 col-sm-12 padding2">
            <a href="{{ asset('/index.php') }}/allevent/{{Auth::user()->uname;}}" class="all_btn" type="submit">View All</a>
          </div>
          <div class="clear"></div>
        </div>
      </div>
...
<p class="openbox" id="<?php echo $hello['idd']; ?>" style="float:left; cursor:pointer; margin-left:15px;"><?php echo $hello['name']; ?></p>
...
<textarea rows="1" onkeydown="if (event.keyCode == 13) document.getElementById('submitbtns<?php echo $hello['idd']; ?>').click()" required="" name="chat" id="chat<?php echo $hello['idd']; ?>" placeholder="Write a reply" class="form-control"></textarea>
...
<button id="submitbtns<?php echo $hello['idd']; ?>" class="onlineclick chat_btn" name="<?php echo $hello['idd']; ?>" type="button"> <span class="glyphicon glyphicon-send"></span>{{trans ('greeting.Reply')}} </button>
