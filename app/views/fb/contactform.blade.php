...
<form onsubmit="return contactsubmit();" action="contact/savecontact" method="get" class="myfirstform">
 @if($errors->any())
...
@if ($errors->has('name')) <div class="alert alert-danger" role="alert"><?php $name=implode('', $errors->all(':message')); if($name=='validation.max.string'){ echo 'Name Maximum length 25'; } else { echo 'Name Required'; } ?></div>@endif
@if ($errors->has('subject')) <div class="alert alert-danger" role="alert"><?php $subject=implode('', $errors->all(':message')); if($subject=='validation.max.string'){ echo 'Subject Maximum length 25'; } else { echo 'Subject Required'; } ?></div>@endif
@if ($errors->has('comment')) <div class="alert alert-danger" role="alert">Message Required</div>@endif
...
<!--           <textarea required id="mess" name="comment" class="form-control" rows="4" cols="78" placeholder="Enter your message here"></textarea> -->
 {{ Form::textarea('comment', '', ['placeholder' => 'Enter your message here','class' => 'form-control','id' => 'mess','rows' => '4','cols' => '78','pattern' => '.*\S+.*','required' => 'required']) }}
        ...
        <div class="controls">
         <button type="submit"  id="mybtn"  class="all_btn" > <span class="glyphicon glyphicon-send"></span> Send Message</button>
        </div>
    </form>
    </div>
