...
<img src="{{ asset('/') }}/public/imagesfb/{{$males}}" width="100%" height="334" alt=""  class="img-rounded img-responsive"/>
<?php
}
...
<a id="remo" class="<?php echo Auth::user()->uname; ?>" style="color:#333; <?php if($photos==0){ echo 'pointer-events:none; color:#018f81;';} ?> @foreach( $profile as $hello) <?php if($hello->cover=='imagesfb/male_cover.jpg' || $hello->cover=='imagesfb/female_cover.jpg'){ echo 'pointer-events:none; color:#018f81;'; } ?> @endforeach" >
<label id="coversimagess" style="<?php if($photos==0){ echo 'background:#14ac9e;';} ?> @foreach( $profile as $hello) <?php if($hello->cover=='imagesfb/male_cover.jpg' || $hello->cover=='imagesfb/female_cover.jpg'){ echo 'background:#14ac9e;'; } ?> @endforeach">
      Remove Cover Photo
...
<img src="{{ asset('/') }}public/imagesfb/{{$male}}" width="170" height="170" alt=""  class="img-circle"/>
<?php
}
...
@endif
    {{ Form::token() }}
          ...
          <input id="coverimagek" name="image" required="" tabindex="1" type="file" onchange="document.getElementById('submitf').click();">
          ...
          <input id="rand" name="rand" type="hidden" value="<?php echo $email = Auth::user()->rand;?>">
                ...
                <input name="submit" id="submitf" tabindex="5" value="submit" type="submit" style="display:none;">
      {{ Form:: close() }}
              </li>
              ...
              <li><a href="{{ asset('/index.php/') }}/gallery"><span class="glyphicon glyphicon-calendar"></span> {{trans ('greeting.Gallery')}}</a></li>
               <li><a href="{{ asset('/index.php/') }}/news"><span class="glyphicon glyphicon-calendar"></span> News Feed</a></li>
...
// id=1;
// setInterval('onlinech(' + id + ')', intervalss);
...
var intervalss = 60 * 60 * 5; // where X is your every X minutes
...
<div id="fleft{{$hello->id}}" class="">
          <div class="u-nformation"><img  alt="iamge" class="img-circle img_wid myimgs" src="{{ asset('') }}/<?php if($allimg) { foreach($allimg as $img){ if($img->user_rand== $hello->user_rand) {  $mm=$img->thumb; echo $mm; break; } else { $mm=''; } } } if($mm==''){ foreach($alluser as $all) { if($all->rand==$hello->user_rand) { if($all->sex=='male') { echo 'imagesfb/male.jpg'; } else { echo 'imagesfb/female.jpg'; } } }  } ?>"> <a href="{{ asset('index.php') }}/profile"><?php  echo Auth::user()->name;?></a>
          <button id="{{$hello->id}}" class="postdelete pull-right"><span class="glyphicon glyphicon-remove"></span></button></div>
        <?php if($hello->path)
        {?>
...
else
{
  ...
  <source src="<?php if($test==0) { echo asset('/'); } ?><?php echo $hello->imagessvideo; ?>" type="video/mp4"/>
  <source src="<?php if($test==0) { echo asset('/'); } ?><?php echo $hello->imagessvideo; ?>" type="video/ogg"/>
<source src="<?php if($test==0) { echo asset('/'); } ?><?php echo $hello->imagessvideo; ?>"/>
...
<param name="allowfullscreen" value="true" />
<param name="allowscriptaccess" value="always" />
<param name="movie" value="<?php if($test==0) { echo asset('/'); } ?><?php echo $newval; ?>" />
<embed src="<?php if($test==0) { echo asset('/'); } ?><?php echo $newval; ?>" type="application/x-shockwave-flash" allowfullscreen="true" allowscriptaccess="always" width="500" height="281">
...
else
{
  ...
  ?>
  <video style="width:100%;" controls>
  <source src="<?php if($test==0) { echo asset('/'); } ?><?php echo $hellos->imagessvideo; ?>" type="video/mp4"/>
  <source src="<?php if($test==0) { echo asset('/'); } ?><?php echo $hellos->imagessvideo; ?>" type="video/ogg"/>
  <source src="<?php if($test==0) { echo asset('/'); } ?><?php echo $hellos->imagessvideo; ?>"/>
  </video>
...
<form onsubmit="return comm({{$hello->id}},this)" name="comments" id="uni{{$hello->id}}" method="post" enctype="multipart/form-data" >
  {{ Form::token() }}
...
<button  id="submitbtn{{$hello->id}}" type="submit" class="buttonss{{$hello->id}} btn send_btn pull-right">{{trans ('greeting.Send')}}</button>
</form>
</div>
...
<h3 id="clickchat"><span style="cursor:move;" class="glyphicon glyphicon-user"></span>  {{trans ('greeting.Chat Messenger')}}</h3>
...
<p class="openbox" id="<?php echo $hello['idd']; ?>" style="float:left; cursor:pointer; margin-left:15px;  margin-right:10px;"><?php echo $hello['name']; ?></p>
<div id="newvalues<?php echo $hello['idd']; ?>"></div>

<div id="boxs<?php echo $hello['idd']; ?>" class="allchatbox" name="<?php echo $hello['idd']; ?>">
<div class="backchat">
<div class="chat_top">
